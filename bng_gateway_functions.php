...
class BNG_Custom_Payment_Gateway extends WC_Payment_Gateway {
    public function __construct() {
        $this->id                   = Config::$pluginId;
        $this->icon                 = Config::$pluginIcon;
        $this->has_fields           = Config::$pluginHasFields;
        $this->order_button_text    = __( Config::$pluginButtonText, Config::$pluginId );
        $this->method_title         = __( Config::$pluginMethodTitle, Config::$pluginId );
        $this->method_description   = __( Config::$pluginDescription, Config::$pluginId );
        $this->gatewayURL           = Config::$pluginUrl;
        ...
        if ( (bgfw_fs()->is_not_paying() || bgfw_fs()->is_free_plan()) && Config::$projectType == 'nmi') {
            $this->supports = array(
                'refunds',
                'tokenization'
            );
    ...
    function admin_options() {
        ?>
            <h3><?php _e('BNG Gateway For WooCommerce', Config::$pluginId); ?></h3>
    ...
    function init_form_fields() {
        ...
        $this->form_fields = array(
            'enabled'           => array(
                'title'         => __( 'Enable/Disable', Config::$pluginId ),
                'type'          => 'checkbox',
                'label'         => __( 'Enable BNG Gateway For WooCommerce', Config::$pluginId ),
                'default'       => 'no',
            ),
            'title'             => array(
                'title'         => __( 'Title', Config::$pluginId ),
                'type'          => 'text',
                'description'   => __( 'This controls the title which the user sees during checkout.', Config::$pluginId ),
                'default'       => __( 'Credit Card / ECheck (BNG)', Config::$pluginId ),
                'desc_tip'      => true,
            ),
            'description'       => array(
                'title'         => __( 'Description', Config::$pluginId ),
                'type'          => 'textarea',
                'description'   => __( 'This controls the description which the user sees during checkout.', Config::$pluginId ),
                'desc_tip'      => true,
                'default'       => __( '', Config::$pluginId ),
            ),
            'instructions'=> array(
                'title'         => __( 'Instructions', Config::$pluginId ),
                'type'          => 'textarea',
                'description'   => __( 'Instructions that will be added to the thank you page.', Config::$pluginId ),
                'desc_tip'      => true,
                'default'       => __( '', Config::$pluginId ),
            ),
            'apikey'            => array(
                'title'         => __( 'API Key (Required)', Config::$pluginId ),
                'type'          => 'password',
                'description'   => __( 'NMI merchant account API key', Config::$pluginId ),
                'desc_tip'      => true,
                'default'       => '',
            ),
            'tokenizationkey'   => array(
                'title'         => __( 'Tokenization Key (Optional)', Config::$pluginId ),
                'type'          => 'password',
                'description'   => __( 'Tokenization Key integrates with collect.js to allow merchants collect sensitive payment data from customers. You can choose to use collect.js by inserting a tokenization key, otherwise, simply leave this part blank.', Config::$pluginId ),
                'desc_tip'      => true,
                'default'       => '',
            ),
            'savepaymentmethodstoggle'  => array(
                'title'                 => __( 'Turn Saved Payment Methods On/Off', Config::$pluginId ),
                'type'                  => 'select',
                'description'           => __( 'Allows you to turn saved payment methods on and off.', Config::$pluginId ),
                'default'               => 'off',
                'desc_tip'              => true,
                'options'               => array(
                                            'on'    => 'On',
            ...
            'transactiontype'   => array(
                'title'         => __( 'Transaction Type', Config::$pluginId ),
                'type'          => 'select',
                'description'   => __( 'Authorize only transaction types works when only credit card payment type is used.', Config::$pluginId ),
                'default'       => 'sale',
                'desc_tip'      => true,
                'options'       => array(
                                'auth' => 'Authorize Only',
            ...
            'paymenttype'       => array(
                'title'         => __( 'Payment Type', Config::$pluginId ),
                'type'          => 'select',
                'description'   => __( 'Allows you to make payments with either credit cards, electronic checks, or both. If credit card and echecks are selected, echeck payments will be authorized and captured.', Config::$pluginId ),
                'default'       => 'both',
                'desc_tip'      => true,
                'options'       => array(
                                'cc' => 'Credit Card Only',
            ...
            'finalorderstatus'  => array(
                'title'         => __( 'Final Order Status', Config::$pluginId ),
                'type'          => 'select',
                'description'   => __( 'This option allows you to set the final status of an order after it has been processed successfully by the gateway.', Config::$pluginId ),
                'default'       => 'Processing',
                'desc_tip'      => true,
                'options'       => array(
            ...
            'redirecturl'       => array(
                'title'         => __( 'Return URL', Config::$pluginId ),
                'type'          => 'text',
                'description'   => '<b>' . __( '*OPTIONAL*', Config::$pluginId ) . '</b> <br />' . __( 'This is the URL the user will be taken to once the sale has been completed. Please enter the full URL of the page. It must be an active page on the same website. If left blank, it will take the buyer to the default order received page.', Config::$pluginId ),
                'desc_tip'      => true,
                'default'       => '',
            ),
        );
        ...
        if ( (bgfw_fs()->is_not_paying() || bgfw_fs()->is_free_plan()) && Config::$projectType == 'nmi') {
            $this->form_fields['savepaymentmethodstoggle'] = array(
                'title'                 => __( 'Turn Saved Payment Methods On/Off', Config::$pluginId ),
                'type'                  => 'select',
                'description'           => __( 'Allows you to turn saved payment methods on and off. Upgrade to the premium version to enable this functionality.', Config::$pluginId ),
                'default'               => 'off',
                'desc_tip'              => true,
                'disabled'              => true,
    ...
    function get_saved_payment_method_list($savedMethods) {
        if (empty($savedMethods) || bgfw_fs()->is_plan('Premium') || Config::$projectType === 'bng') {
    ...
    function receipt_page( $order_id ) {
        if ( function_exists('wcs_is_subscription') ) {
            $order = wcs_is_subscription($order_id) ? new WC_Subscription( $order_id ) : new WC_Order( $order_id );
        }   
        else {
        ...
        if ($this->apikey != '') {
            ...
            if (bgfw_fs()->is_plan('Premium') || Config::$projectType == 'bng') {
                if ( is_user_logged_in() && $this->savepaymentmethodtoggle == 'on' ) {
                    ...
                    if (!empty($temp_payment_tokens)) {
                        ...
                        foreach ($payment_tokens as $pt) {
                            ...
                            $type = $pt->get_type();
                            if (!empty($sub_billingId) && $sub_billingId === $billing_id) $highlight = true;
    ...
    function add_admin_change_payment_method_form($payment_meta, $subscription) {
        if ( class_exists('WC_Subscriptions') && (bgfw_fs()->is_plan('Premium') || Config::$projectType === 'bng') ) {
            $vaultId = empty($subscription->get_meta('_bng_gateway_vault_id')) ? 'null' : $subscription->get_meta('_bng_gateway_vault_id');
            $billingId = empty($subscription->get_meta('_bng_gateway_billing_id')) ? 'null' : $subscription->get_meta('_bng_gateway_billing_id');

            $payment_meta[ $this->id ] = array(
                'post_meta' => array(
                    '_bng_gateway_vault_id' => array(
                        'value' => $vaultId,
                        'label' => 'BNG Gateway Customer ID',
                    ),
                    '_bng_gateway_billing_id' => array(
                        'value' => $billingId,
                        'label' => 'BNG Gateway Billing ID',
                    ),
                ),
            );
        }
            ...
            if ($acctScreen || (!empty($order) && $order->get_status() != 'Completed')) {
                ...
                if($result['result'] == 1) {
                    ...
                    if ( !$acctScreen || (function_exists('wcs_is_subscription') && !wcs_is_subscription($orderid))) {
                        ...
                        $response = wp_remote_post(Config::$pluginUrl, $args);
                    ...
                    if ($result['result'] == 1) $orderStatus = "success";
                }
            }
        }
        else {
            ...
            if ($order->get_status() != 'Completed') {
                ...
                if($result['result'] == 1) {
                    ...
                    if (empty($customerTokens)) {
                        ...
                        if (empty($customervaultid) || empty($billingid)) $donotcreate = true;
                    }
                    else {
                        if (empty($billingid)) {
                            $donotcreate = true;
                            ...
                            foreach($customerTokens as $token) {
                                if ($token->get_token() == $billingid) $donotcreate = true;
                            }
                        }
                    }
                    if ($donotcreate === false) $newPmToken = bng701_create_woocommerce_payment_token($billingid, $customervaultid, $this->apikey);
                }
            }
        }

        if ( class_exists('WC_Subscriptions') && (Config::$projectType == 'bng' || bgfw_fs()->is_plan('Premium')) ) {
    ...
    function collectjs_to_direct_post_request() {
        ...
        if ($paymentType == 'check' && $this->transactiontype == 'auth') $transactionType = 'sale';
        ...
        if (!empty($hasVault)) {
            if (isset($_POST['bng_save_paymentmethod'])) $savePm = $_POST['bng_save_paymentmethod'];
            if (isset($_POST['bng_woo_token'])) $woo_token = $_POST['bng_woo_token'];
            ...
            if ($hasVault == 'true') {
                ...
                if (empty($vaultId) && !empty($paymentTokens)) $vaultId = current($paymentTokens)->get_meta('vaultid');
            }
        ...
        if (function_exists('wcs_is_subscription') && wcs_is_subscription($order_id) && $orderTotal == 0) $query .= bng701_generic_collectjs_query($order, $this->apikey, $paymentType, $orderTotal, $payment_token, false);
        else if ( $acctScreen === true ) $query .= bng701_generic_collectjs_query($order, $this->apikey, $paymentType, $orderTotal, $payment_token, false, true);
        else {
        ...
        if ($paymentType == 'check') $result['sec-code'] = 'WEB';
    ...
    function process_refund( $order_id, $amount = null, $reason = '' ) {
        ...
        if (is_wp_error($response)) {
            ...
            if ($result_id == 1) {
                ...
                if ($toVoid === true) $note = "Transaction voided because it was pending settlement";
                else if ($amount == $total) $note = "This transaction was refunded in full";
        ...
        // Add helpful notes
        if (!empty($reason)) $note = "$note\n$reason";
        $order->add_order_note( __( $note, Config::$pluginId ) );
    ...
    function process_subscription_payment( $order, $amount_to_charge ) {
        try {
            $billingId = $order->get_meta('_bng_gateway_billing_id');
            $vaultId = $order->get_meta('_bng_gateway_vault_id');

            bng701_verify_nmi_payment_method_details($billingId, $vaultId, $this->apikey);

            if (empty($cc_number)) $paymentType = 'check';
            ...
            if ($responses['response'] != "1") return new WP_Error( $responses['responsetext'] );
            return $responses;
        }
        catch(Exception $ex) {   
            $this->logger->handle(time(), 'error', __($ex->getMessage(), Config::$pluginId), array());
    ...
    function validate_admin_paymentmethod_change( $payment_meta ) {
        if ( ! isset( $payment_meta['post_meta']['_bng_gateway_vault_id']['value'] ) || empty( $payment_meta['post_meta']['_bng_gateway_vault_id']['value'] ) ) {
            $this->logger->handle(time(), 'error', __('Customer vault id is required to update payment method', Config::$pluginId), array());
            throw new Exception( 'A customer vault identifier is required.' );
        }
        
        if ( ! isset( $payment_meta['post_meta']['_bng_gateway_billing_id']['value'] ) || empty( $payment_meta['post_meta']['_bng_gateway_billing_id']['value'] ) ) {
            $this->logger->handle(time(), 'error', __('Billing id is required to update payment method', Config::$pluginId), array());
            throw new Exception( 'A billing identifier is required.' );
        }

        // if both values exists, then verify it exists in the nmi gateway
        $vaultId = $payment_meta['post_meta']['_bng_gateway_vault_id']['value'];
        $billingId = $payment_meta['post_meta']['_bng_gateway_billing_id']['value'];
        ...
        if ('null' != $vaultId && 'null' != $billingId) {
            try {
                ...
                $this->logger->handle(time(), 'error', __($ex->getMessage(), Config::$pluginId), array());
                throw new Exception( __( $ex->getMessage(), Config::$pluginId) );
            }
        }
    }
    ...
    function validate_options() {
        $response = $this->validate_api_key( $_POST["woocommerce_bng_gateway_apikey"] );
        $valid = true;
        $error = $msg = '';

        if (isset($response['customer_vault'])) {
            // connection passed, process and save options
            if ($_POST["woocommerce_bng_gateway_paymenttype"] == "ach" && $_POST["woocommerce_bng_gateway_transactiontype"] == "auth") {
                $valid = false;
                $error = "Invalid selected options. Cannot use authorization only transaction type for Echeck payments";
            }
            elseif ($_POST["woocommerce_bng_gateway_paymenttype"] == "both" && $_POST["woocommerce_bng_gateway_transactiontype"] == "auth") {
...
function bng701_add_order_capture_charge_action( $actions ) {
    ...
    if ($doNotAddCaptureAction === true ) {
        if ( !empty( get_post_meta( $theorder->get_id(), 'order_charge_captured', true)) ) {
            ...
            $theorder->add_order_note( __( "Authorization has been captured using NMI account", Config::$pluginId ) );
            ...
            if (isset($transaction['action'][1]['response_text'])) {
                ...
                $theorder->add_order_note( __( $note, Config::$pluginId ) );
            }
    ...
    $actions['capture_charge_action'] = __( 'Capture Charge', Config::$pluginId );
    return $actions;
}
...
function bng701_complete_order($order, $status, $result, $trans_type, $bngGatewayObj, $isAcctScreen = false) {
    ...
    if ($status == 'success') {
        if ($isAcctScreen) {
            ...
            if ( function_exists('wcs_is_subscription') && !wcs_is_subscription($order) || !function_exists('wcs_is_subscription')) {
                ...
                if ($trans_type == 'auth') {
                    ...
                    $order->add_order_note( __( 'BNG Gateway payment authorization completed.', Config::$pluginId ) );
                    $order->set_transaction_id($result['transaction-id']);
                    $order->save();
                }
                else {
                    $order->add_order_note( __( 'BNG Gateway payment completed.', Config::$pluginId ) );
                ...
                if ($oncomplete_action == 'Completed') {
                    ...
                    $order->update_status('completed', 'Successful payment by the BNG Three Step Gateway');
                }
                elseif ($oncomplete_action == 'Ready to Ship') {
                    //flag the order as completed in the eyes of woo
                    $order->update_status('ready-to-ship', 'Successful payment by the BNG Three Step Gateway');
                }

                if ($trans_type == 'auth') {
                    $order->update_status('on-hold');
                    $order->add_order_note( __( 'Awaiting payment capture.', Config::$pluginId ) );
                }
                ...
                if (isset($result['sec-code'])) {
                    $order->add_order_note( __( 'This transaction was paid using ECheck. Delay shipping until payment clears.', Config::$pluginId ) );
                }

                //display confirmation message
                wc_add_notice(__('Your order is complete! Thank you!', Config::$pluginId), $status);
            }
        }
    }
    else {
        ...
        if ($isAcctScreen) {
            ...
            $order->add_order_note( __( 'Failure: Payment method could not be changed.', Config::$pluginId ) );
            $order->add_order_note( __( "Reason - {$dsp_error}", Config::$pluginId ) );
            
            wc_add_notice( __( "Failure: Payment method could not be changed on subscription {$order->get_id()}.", Config::$pluginId ), $status );
        }
        else {
            // Add helpful notes
            ...
            wc_add_notice(__('An error occurred completing order: ' . $dsp_error, Config::$pluginId), $status);
    ...
    // redirection
    if ($isAcctScreen) {
        if ( isset($_GET['complete']) ) WC_Form_Handler::add_payment_method_action();
    }
    else if ( function_exists('wcs_is_subscription') && wcs_is_subscription($order)) {
        wp_safe_redirect( wc_get_endpoint_url( 'subscriptions', '', wc_get_page_permalink( 'myaccount' ) ) );
        exit();
        ...
        if ($redirect_url == '') $redirect_url = $order->get_checkout_order_received_url();
...
function bng701_process_capture_charge_action($order) {
    ...
    if ($responses['response'] == "1") {
        ...
        $order->add_order_note( __( "Authorization capture successful", Config::$pluginId ) );
        $note = bng701_get_order_completion_notes('success', $result);
        $order->add_order_note( __( $note, Config::$pluginId ) );
        ...
        $order->add_order_note( __( "Authorization capture failed", Config::$pluginId ) );
        $order->add_order_note( __( $dsp_error, Config::$pluginId ) );
...
function bng701_html($savepaymentmethodtoggle, $tokenizationkey='', $data='', $order_id='', $paymentMethods='') {
    ...
    $hasVault = '';
    $pageMessage = 'Pay via the BNG Payment Gateway';

    if (Config::$projectType == 'bng' || bgfw_fs()->is_plan('Premium') ) {
        // to display save payment method checkbox
        if (is_user_logged_in() && $savepaymentmethodtoggle == 'on') {
            $showSavePmLater = true;
            if (empty($order_id)) $forceSavePm = true;
        }
        ...
        if ( function_exists('wcs_is_subscription') && wcs_is_subscription($order_id) ) $pageMessage = "Select Desired Payment Method via the BNG Payment Gateway";
        
        // find out if customer has nmi vault or not
        $wc_tokens = WC_Payment_Tokens::get_customer_tokens( get_current_user_id() );
        if (!empty($wc_tokens)) $hasVault = 'true';
        else $hasVault = 'false';
    }
        ...
        <form name="bng_submitPayment" id="bng_submitPayment" action="" method="POST">
            ...
            <div class="bng-form-group active">
                <div class="preferred_payment_method">
                    ...
                    <input type="radio" id="ach" name="paymentType" class="display_ach cc_preferred">
...
function bng701_cleanTheData($data, $datatype = 'none') {
    ...
    switch ($datatype) {
        ...
        case 'url':
            ...
            break;
        case 'email':
            if (filter_var($data, FILTER_VALIDATE_EMAIL) === false) $data = "";
...
function bng701_pw_load_scripts() {
    wp_enqueue_script('my_script', plugin_dir_url(__FILE__) . 'js/my_script.js', array('jquery'));
    ...
    wp_localize_script( 'backThatUp', 'frontendajax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' )));

    wp_enqueue_style('my_styles', plugin_dir_url(__FILE__) . 'css/my_styles.css');

    wp_enqueue_script( 'bng701_ajax_custom_script',  plugin_dir_url(__FILE__) . 'js/stepOne.js', array('jquery') );
    wp_localize_script( 'bng701_ajax_custom_script', 'frontendajax', array( 'ajaxurl' => admin_url( 'admin-ajax.php' )));
    wp_enqueue_script( 'bng701_ajax_custom_script1',  plugin_dir_url(__FILE__) . 'js/deletePaymentMethod.js', array('jquery') );
...
function bng701_stepOne_addBilling() {
    ...
    if ($paymentType == 'check' && $transactiontype == 'auth') $transactiontype = 'sale';
    ...
    if (is_wp_error($response)) {
        ...
        else wp_send_json_error($full_response['result-text'], 400);
    }
}
...
function bng701_stepOne() {
    ...
    if (empty($isAcctScreen)) $referrer.= "?order=".$orderid;
    else {
    ...
    if (!empty($tokenId)) {
        ...
        if (function_exists('wcs_is_subscription') && wcs_is_subscription($orderid) && $ordertotal == 0) {
            ...
            if (is_wp_error($response)) {
                ...
                if ($full_response['result'] == '1') echo $formURL."||".$rc."||".$tid."||".$ac."||".$ar;
                else wp_send_json_error($full_response['result-text'], 400);
            }
        }
    }
    elseif ($savepaymentmethod == 'Y' ) {
        ...
        if (is_wp_error($response)) wp_send_json_error($response->get_error_message(), $response->get_error_code());
        else {
            $xml = simplexml_load_string($response['body'], "SimpleXMLElement", LIBXML_NOCDATA);
            $json = json_encode($xml);
            $full_response = json_decode($json, TRUE);
        ...
        if (is_wp_error($response)) {
            ...
            if (isset($full_response['form-url'])) echo $full_response['form-url'];
            else wp_send_json_error($full_response['result-text'], 400);
        } 
    }    
...
function bng701_deletePaymentMethod() {
    ...
    if (!empty($security) && !$isAccountScreen) check_ajax_referer( 'delete-pm-nonce', $security, false );
    ...
    if (is_wp_error($response)) {
        ...
        if ($resultId == 1) {
            if (!$isAccountScreen) WC_Payment_Tokens::delete( $tokenId );
            echo $resultId;
        }
        else {
    ...
    if (!empty($temp_payment_tokens)) {
        ...
        if (empty($nmiCustomerDetails)) {
            ...
            foreach ($temp_payment_tokens as $payment_token) {
                ...
                $found = array_filter( $nmi_billings, function( $billing ) use ($bid) { return $billing['@attributes']['id'] == $bid; });
                
                // if woo token is not in nmi, delete from woo
                if (empty($found)) WC_Payment_Tokens::delete( $payment_token->get_id() );
            }
...
function bng701_create_woocommerce_payment_token($billingId, $vaultId, $apikey, $paymentDetails = '') {    
    if (empty($paymentDetails)) $paymentDetails = bng701_getPMDetailsByBillingId($billingId, $apikey)['customer']['billing'];
...
function bng701_update_paymentmethod_subscription($newPmToken, $order, $billingId) {
    if (Config::$projectType == 'bng' || bgfw_fs()->is_plan('Premium') ) {
        ...
        if (wcs_is_subscription($orderid) || wcs_order_contains_resubscribe( $order ) || wcs_order_contains_renewal( $order )) {
            $sub_id = $orderid;
            if ( wcs_order_contains_renewal( $order ) ) $sub_id = (int)$order->get_meta('_subscription_renewal');
        }
        ...
        if (empty($newPmToken)) {
            foreach($payment_tokens as $pt) {
                if ($pt->get_token() == $billingId) $newPmToken = $pt;
            }
        }
        ...
        if (wcs_is_subscription($orderid) || wcs_order_contains_resubscribe( $order ) || wcs_order_contains_subscription( $order ) || wcs_order_contains_renewal( $order )) {
            ...
            if (wcs_order_contains_renewal( $order )) $order = wcs_get_subscription($sub_id);
            
            if (wcs_order_contains_subscription($order)) {
                foreach ( wcs_get_subscriptions_for_order($orderid) as $subscription) {
                    $subscription->set_payment_method(Config::$pluginId, $payment_meta);
                    $subscription->save();
                ...
                $order->set_payment_method(Config::$pluginId, $payment_meta);
                $order->save();
            ...
            if ( wcs_is_subscription( $order ) && WC_Subscriptions_Change_Payment_Gateway::will_subscription_update_all_payment_methods( $order ) ) {
                $update_all = WC_Subscriptions_Change_Payment_Gateway::update_all_payment_methods_from_subscription( $order, Config::$pluginId );
...
function bng701_payment_method_change_notification($token, $order) {
    ...
    $order->add_order_note( __("Recurring payment method has been changed to {$msg} ending in '{$last4}'.", Config::$pluginId));
    wc_add_notice( __( "Payment method successfully changed on subscription {$order->get_id()}.", Config::$pluginId ) );
}
...
function bng701_remove_payment_meta_from_suscription($subscription) {
    ...
    $subscription->delete_meta_data( '_bng_gateway_vault_id' );
    $subscription->delete_meta_data( '_bng_gateway_billing_id' );
    $subscription->save_meta_data();
    $subscription->add_order_note( __("Recurring payment method has been removed.", Config::$pluginId));
    wc_add_notice(__("Payment method has been deleted from subscription - {$id}", Config::$pluginId));
}
...
function bng701_generic_collectjs_query($order, $apikey, $paymentType, $orderTotal, $payment_token = '', $include_total = true, $add_billing = false) {
    ...
    if ($include_total == true) $query .= "&amount=" . urlencode($orderTotal);
    $query .= "&payment=" . urlencode($paymentType);
    $query .= "&currency=" . urlencode(get_woocommerce_currency());
    if ($paymentType == 'check') $query .= "&sec_code=WEB";
    ...
    if ($add_billing === false) {
        ...
        if (!empty($email)) $query .= "&customer_receipt=true";
        // Billing Information
...
function bng701_getCustomerDetails($vaultId, $apiKey) {
    ...
    if (isset($response['customer_vault']['customer'])) return $response['customer_vault']['customer'];
}
...
function bng701_getPMDetailsByBillingId($billingid, $apikey) {
    ...
    if (isset($response['customer_vault'])) return $response['customer_vault'];
}
...
function bng701_getTransaction($trans_id, $apikey) {
    ...
    if (isset($response['transaction'])) return $response['transaction'];
}
...
function bng701_doCurl($query) {
    ...
    curl_setopt($ch, CURLOPT_SSL_VERIFYPEER, 0); // todo remove in production
